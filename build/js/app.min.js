...
const cambiarSeccion=()=>{const seccion=document.querySelectorAll("A");seccion.forEach(enlace=>{enlace.addEventListener("click",(e)=>{pagina=e.target.dataset.paso;eliminarSeccion();mostrarSeccion();document.documentElement.style.setProperty('--animate-duration','.5s');document.documentElement.style.setProperty('--animate-duration', '.5s');document.documentElement.style.setProperty('--animate-delay', '0s');document.documentElement.style.setProperty('--animate-timing', 'ease-in-out');document.documentElement.style.setProperty('--animate-iteration-count', '1');document.documentElement.style.setProperty('--animate-direction', 'normal');document.documentElement.style.setProperty('--animate-fill-mode', 'none');document.documentElement.style.setProperty('--animate-play-state', 'running');document.querySelector(`[data-paso="${pagina}"]`).scrollIntoView({behavior: "smooth",block: "start"});})})};const btnSwitch=document.querySelector("#switch");btnSwitch.addEventListener("click",()=>{document.body.classList.toggle("dark");btnSwitch.classList.toggle("active")});const input_number = document.querySelector("#input_cantidad");const input_select= document.querySelector("#select_size");let result = "none";let cantidad = 0;input_select.addEventListener("change", (e) => {result = e.target.value;btnAnimacion();});input_number.addEventListener("change", (e) => {cantidad = e.target.value;btnAnimacion();});const btnAnimacion = () => {if (cantidad > 0 && result != "none") {document.querySelector("#boton_compra").classList.add("button_animate");setTimeout(() => {document.querySelector("#boton_compra").classList.remove("button_animate");}, 1000);}};